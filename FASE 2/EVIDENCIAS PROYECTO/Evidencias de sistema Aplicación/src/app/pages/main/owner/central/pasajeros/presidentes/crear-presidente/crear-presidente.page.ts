...
@Component({
  ...
  templateUrl: './crear-presidente.page.html',
  styleUrls: ['./crear-presidente.page.scss'],
})
export class CrearPresidentePage implements OnInit {
  ...
  form = new FormGroup({
    ...
    tipo_usuario: new FormControl('1'),
  ...
  async setUserInfo(uid: string) {
    if (this.form.valid) {
      ...
      this.firebaseSvc.setDocument(path, this.form.value).then(async res => {
        this.utilsSvc.routerLink('/main/owner/central/pasajeros/presidentes');
        this.form.reset();
        this.utilsSvc.presentToast({
