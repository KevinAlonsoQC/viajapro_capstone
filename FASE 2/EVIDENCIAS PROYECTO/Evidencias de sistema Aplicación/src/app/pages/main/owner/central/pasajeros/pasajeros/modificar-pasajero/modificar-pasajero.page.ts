...
@Component({
  ...
  templateUrl: './modificar-pasajero.page.html',
  styleUrls: ['./modificar-pasajero.page.scss'],
})
export class ModificarPasajeroPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.route.params.subscribe(async params => {
      ...
      try {
        ...
        if (usuarioObtenido && usuarioObtenido['tipo_usuario'] === '3' && this.usuario.tipo_usuario === '0') {
          console.log('Usuario válido:', usuarioObtenido);
          this.pasajero = usuarioObtenido;
  ...
  async submit() {
    if (this.form.valid) {
      ...
      try {
        ...
        if (this.form.value.img_usuario !== this.pasajero.img_usuario) {
          //Si la imagen es default, entonces creará una nueva cosa en el storage para evitar errores
          if (this.pasajero.img_usuario == 'https://ionicframework.com/docs/img/demos/avatar.svg' || this.pasajero.img_usuario == '') {
            let imagePath = `usuario/${this.pasajero.uid}}/${Date.now()}`;
            ...
            let imagePath = await this.firebaseSvc.getFilePath(this.pasajero.img_usuario);
            let imageUrl = await this.firebaseSvc.uploadImage(imagePath, this.form.value.img_usuario);
            this.form.controls.img_usuario.setValue(imageUrl)
          }

          this.pasajero.img_usuario = this.form.value.img_usuario
        }

        await this.firebaseSvc.updateDocument(`usuario/${this.pasajero.uid}`, this.form.value);
        this.pasajero = {...this.pasajero, ...this.form.value}

        // Rellenar el formulario con los datos del usuario autenticado
        this.form.patchValue({
          name: this.pasajero.name,
          email: this.pasajero.email,
          telefono_usuario: this.pasajero.telefono_usuario,
          img_usuario: this.pasajero.img_usuario,
        });
